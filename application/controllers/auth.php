...
class Auth extends CI_Controller {
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
        	...
        	$this->data['header']     = 'layouts/header';
       		$this->data['last_line']  = 'layouts/last_line';
        	$this->data['sidebar']    = 'layouts/sidebar';
        	$this->data['footer']	  = 'layouts/footer';
		...
		else
		{
			...
			if ($change)
			{
				...
				$this->session->set_flashdata('msg', 'Your password has been changed successfully. Please login with your new details.');
				$this->logout();
			}
			else
			{
	...
	function forgot_password()
	{
		...
		else
		{
			...
			if ($forgotten)
			{
                /* reset code is generated. Send it via email */
                $this->email->from('donotreply@holidaysrating.com', 'Holidaysrating.com');
                $this->email->to($forgotten['identity']);         
                $this->email->subject('Reset Password');
                $this->email->set_mailtype('html');
                $data['identity'] = $forgotten['identity'];
                $data['forgotten_password_code'] = $forgotten['forgotten_password_code']; 
                $this->email->message($this->load->view('auth/email/forgot_password.tpl.php', $data, TRUE));	
                $this->email->send();
                $this->session->set_flashdata('msg', 'Reset code is sent to your email');
    ...
    function register()
	{
	    if($this->ion_auth->logged_in())
        {
            $this->session->set_flashdata('msg', 'You are already registered');
            redirect('home', 'refresh');
        }
        
		$this->form_validation->set_rules($this->Ion_auth_model->rules);
		...
		{
		    // retrieve admin's id
            $this->db->select('id')->where('email', $this->config->item('admin_email'));
            $sender = $this->db->get('users')->row();
            
            // retrieve recipient's id
            $this->db->select('id')->where('email', $email);
            $recipient = $this->db->get('users')->row();
            
            $data = array(
                'from'      => $sender, 
                'to'        => $recipient,
                'subject'   => 'Welcome to Holidaysrating.com!',
                'content'   => 'Welcome to Holidaysrating.com!'
            );
    ...
    function fb_login()
    {
        ...
        if($this->user)
        {
            try
            {
                ...
                $this->logoutUrl = $this->facebook->getLogoutUrl(array("next" => site_url("auth/logout")));
            }
    ...
    public function fb_login_success()
    {
        $this->Ion_auth_model->process_fb_login();
    ...
    {
        $email = $this->input->post('email');
        $status = $this->Ion_auth_model->email_exists($email);
